...
<ctl:package xmlns:ctl="http://www.occamlab.com/ctl"
	...
	<ctl:function name="tns:run-ets-${ets-code}">
		...
		<ctl:code>
			<xsl:variable name="controller"
				select="tng:new($outputDir)" />
			<xsl:copy-of
				select="tng:doTestRun($controller, $testRunArgs)" />
		</ctl:code>
	</ctl:function>
	...
	<ctl:test name="tns:Main">
		<ctl:assertion>The test subject satisfies all applicable constraints.
		</ctl:assertion>
		<ctl:code>
			<xsl:variable name="form-data">
				<ctl:form method="POST" width="800" height="600"
					xmlns="http://www.w3.org/1999/xhtml">
					<h2>DGIWG WMTS 1.0.0 Conformance Test Suite</h2>
			   ...
			   background-color:#F0F8FF; border-style: solid; border-width: medium; padding:4px">
							...
							<p>
								...
								<input id="wmts-vector-false" name="wmts-vector"
									type="radio" value="no" checked="checked">
									<label for="wmts-vector-false"> no</label>
								</input>
								<br />
								<input id="wmts-vector-true" name="wmts-vector"
			...
			<xsl:variable
				name="getFeatureInfoExceptionInEnglishLanguage">
				<ctl:call-function
					name="interactive:featureInfoExceptionInEnglishLanguage">
			...
			<xsl:variable name="test-run-props">
				<properties version="1.0">
					...
					<entry key="vector">
						<xsl:choose>
							<xsl:when
								test="normalize-space($form-data/values/value[@key='wmts-vector']) = 'yes'">
			...
			<xsl:variable name="testRunDir">
				<xsl:value-of
					select="tec:getTestRunDirectory($te:core)" />
			</xsl:variable>
			<xsl:variable name="test-results">
				<ctl:call-function name="tns:run-ets-${ets-code}">
					<ctl:with-param name="testRunArgs"
						select="$test-run-props" />
					<ctl:with-param name="outputDir"
						select="$testRunDir" />
				</ctl:call-function>
			</xsl:variable>
			...
			<ctl:message>
				<xsl:value-of
					select="saxon:transform(saxon:compile-stylesheet($summary-xsl), $test-results)" />
			...
			<xsl:if
				test="xs:integer($test-results/testng-results/@failed) gt 0">
				<xsl:for-each
					select="$test-results//test-method[@status='FAIL' and @description='prerequisite']">
					<ctl:message>
						Test prerequisite
				...
				<xsl:for-each
					select="$test-results//test-method[@status='FAIL' and not(@is-config='true')]">
					<ctl:message>
						Test method
						<xsl:value-of select="./@name" />
						:
	...
	<xsl:template name="tns:testng-report">
		...
		<xsl:variable name="stylesheet"
			select="tec:findXMLResource($te:core, '/testng-report.xsl')" />
		<xsl:variable name="reporter"
			select="saxon:compile-stylesheet($stylesheet)" />
		...
		<xsl:copy-of
			select="saxon:transform($reporter, $results, $report-params)" />
	</xsl:template>
</ctl:package>
