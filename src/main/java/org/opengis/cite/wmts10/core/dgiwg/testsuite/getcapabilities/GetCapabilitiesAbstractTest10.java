...
public class GetCapabilitiesAbstractTest10 extends AbstractBaseGetCapabilitiesFixture {
	...
	/**
	 *  DGIWG WMTS 1.0, Requirement 10 If a WMTS server is providing services to a coalition mission federated network, in support of operations or an exercise, 
    ...
    @Test(groups = {"If a WMTS server is providing services to a coalition mission federated network, in support of operations or an exercise, it shall include the following information in the <abstract> element of the service metadata: \"This service implements the WMTS 1.0 STANAG 6523 Ed.2 profile\". The provision of these metadata elements are optional for a WMTS server which is providing services across one single non-mission network."},
    		description = "Checks abstract.")
