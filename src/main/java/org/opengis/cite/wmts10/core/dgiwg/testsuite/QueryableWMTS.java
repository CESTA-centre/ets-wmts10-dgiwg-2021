...
/**
 * The DGIWG WMTS profile requires that a conforming implementation satisfies the
 * requirements of the <strong>Queryable WMTS</strong> conformance class in
 * accord with the base WMTS standard. See Table 1, cl. 6.2: Requirement 1.
 * 
 * <p>
 ...
 *      "https://portal.dgiwg.org/files/68271">DGIWG -
 *      Web Map Tile Service 1.0 Profile (DGIWG-124, Ed. 1.0.0)</a>
 * @see <a target="_blank" href=
 *      "http://portal.opengeospatial.org/files/?artifact_id=35326">OpenGIS&#174;
 *      Web Map Tile Service Implementation Specification, Version 1.0.0</a>
 */
public class QueryableWMTS {

    private static final Logger LOGR = Logger.getLogger(QueryableWMTS.class.getPackage().getName());
    ...
    /** Test run parameter: Run tests for 'Queryable WMTS' (boolean). */
    private static final String QUERYABLE = "queryable";
    /** JAX-RS Client component. */
    private Client httpClient;
    /** Endpoint of base WMTS test run controller. */
    private URI baseTestRunController;
    /** URI that refers to the WMTS capabilities document for the IUT. */
    private String wmtsCapabilitiesRef;
    ...
    public void initTestFixture(ITestContext testContext) {
        ...
        this.wmtsCapabilitiesRef = testContext.getSuite().getParameter(TestRunArg.WMTS.toString());
    }
    ...
    public void discoverBaseControllerEndpoint(ITestContext testContext) {
        ...
        if (null == this.baseTestRunController) {
            try {
                ...
                LOGR.warning("WMTS controller endpoint not available. " + e.getMessage());
                throw new SkipException(e.getMessage());
            }
        }
        LOGR.info("Using WMTS controller endpoint at " + this.baseTestRunController.toString());
    }

    /**
     * Runs the OGC WMTS test suite by submitting a (GET) request to the test run
     * controller. The following test run arguments are set:
     * <ul>
     * <li>capabilities-url : A URI that refers to a WMTS capabilities
     * document</li>
     ...
     *             run results.
     */
    @Test(description = "See DGIWG-124: Table 1, Requirement 1") //TODO : check requirement number
    public void invokeBaseTestSuite() throws IOException {
        ...
        args.putSingle(CAPABILITIES_URL, this.wmtsCapabilitiesRef);
    ...
    File saveEntityToFile(ClientResponse rsp) throws IOException {
        File destFile = File.createTempFile("wmtsResults-", ".xml");
     ...
     * URI for the base WMTS test run controller. This controller executes the
     ...
     * service and <code>{version}</code> is the version of the WMTS test suite.
     * 
     * <pre>
     * {tes}/rest/suites/wmts/{version}/run
     * </pre>
     * 
     * @param tesEndpoint
     *            An 'http' URI that refers to the test execution service.
     * @return The URI for the standard WMTS test run controller.
     */
    URI discoverTestRunController(URI tesEndpoint) {
        ...
        // find WMTS test run controller
        String xpath = "//html:li/html:a[starts-with(@id,'wmts')]/@href";
        XdmValue xdmVal;
        ...
        if (xdmVal.size() == 0) {
            throw new RuntimeException(String.format("WMTS test suite is not listed at %s", targetURI));
        }
